...
pub trait WalletSigner: Signer {
    ...
    /// ECDSA-sign a 32-byte digest with the identity key.
    ///
    /// Returns the compact (64-byte) signature. Used for Spark protocol
    /// signatures (e.g. `SendLeafKeyTweak.signature`).
    fn sign_ecdsa_digest_compact(&self, digest: &[u8; 32]) -> [u8; 64];
