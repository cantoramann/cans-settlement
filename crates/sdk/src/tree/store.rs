...
pub struct TreeNode {
    ...
    /// Serialized CPFP refund transaction.
    pub refund_tx: Option<Vec<u8>>,

    /// Serialized direct transaction (watchtower broadcast path).
    pub direct_tx: Option<Vec<u8>>,

    /// Serialized direct refund transaction.
    pub direct_refund_tx: Option<Vec<u8>>,

    /// Serialized direct-from-CPFP refund transaction.
