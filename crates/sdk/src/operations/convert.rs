...
pub fn proto_to_tree_node(node: &spark::TreeNode) -> Option<TreeNode> {
    ...
    Some(TreeNode {
        ...
        refund_tx: optional_bytes(&node.refund_tx),
        direct_tx: optional_bytes(&node.direct_tx),
        direct_refund_tx: optional_bytes(&node.direct_refund_tx),
        direct_from_cpfp_refund_tx: optional_bytes(&node.direct_from_cpfp_refund_tx),
