// This file is @generated by prost-build.
/// Challenge represents the core challenge data
#[allow(clippy::large_enum_variant)]
#[allow(clippy::derive_partial_eq_without_eq)]
#[derive(Clone, PartialEq, Eq, Hash, ::prost::Message)]
pub struct Challenge {
    /// Protocol version for backward compatibility
    #[prost(int32, tag = "1")]
    pub version: i32,
    /// Timestamp when challenge was issued (UTC Unix seconds)
    #[prost(int64, tag = "2")]
    pub timestamp: i64,
    /// Random nonce to prevent replay attacks (32 bytes)
    #[prost(bytes = "bytes", tag = "3")]
    pub nonce: ::prost::bytes::Bytes,
    /// The public key this challenge is intended for (uncompressed secp256k1
    /// public key)
    #[prost(bytes = "bytes", tag = "4")]
    pub public_key: ::prost::bytes::Bytes,
}
/// ProtectedChallenge wraps a Challenge with a server HMAC
#[allow(clippy::large_enum_variant)]
#[allow(clippy::derive_partial_eq_without_eq)]
#[derive(Clone, PartialEq, Eq, Hash, ::prost::Message)]
pub struct ProtectedChallenge {
    /// Protocol version for backward compatibility
    #[prost(int32, tag = "1")]
    pub version: i32,
    /// The core challenge data
    #[prost(message, optional, tag = "2")]
    pub challenge: ::core::option::Option<Challenge>,
    /// Server's HMAC of the Challenge
    #[prost(bytes = "bytes", tag = "3")]
    pub server_hmac: ::prost::bytes::Bytes,
}
/// Request to initiate an authentication challenge
#[allow(clippy::large_enum_variant)]
#[allow(clippy::derive_partial_eq_without_eq)]
#[derive(Clone, PartialEq, Eq, Hash, ::prost::Message)]
pub struct GetChallengeRequest {
    /// Client's public key (uncompressed secp256k1 public key)
    #[prost(bytes = "bytes", tag = "1")]
    pub public_key: ::prost::bytes::Bytes,
}
/// Response containing the protected challenge
#[allow(clippy::large_enum_variant)]
#[allow(clippy::derive_partial_eq_without_eq)]
#[derive(Clone, PartialEq, Eq, Hash, ::prost::Message)]
pub struct GetChallengeResponse {
    /// The protected challenge from the server
    #[prost(message, optional, tag = "1")]
    pub protected_challenge: ::core::option::Option<ProtectedChallenge>,
}
/// Request to verify a signed challenge
#[allow(clippy::large_enum_variant)]
#[allow(clippy::derive_partial_eq_without_eq)]
#[derive(Clone, PartialEq, Eq, Hash, ::prost::Message)]
pub struct VerifyChallengeRequest {
    /// The protected challenge from the server
    #[prost(message, optional, tag = "1")]
    pub protected_challenge: ::core::option::Option<ProtectedChallenge>,
    /// Client's secp256k1 signature of the Challenge
    #[prost(bytes = "bytes", tag = "2")]
    pub signature: ::prost::bytes::Bytes,
    /// Client's public key (uncompressed secp256k1 public key)
    #[prost(bytes = "bytes", tag = "3")]
    pub public_key: ::prost::bytes::Bytes,
}
/// Response after successful authentication
#[allow(clippy::large_enum_variant)]
#[allow(clippy::derive_partial_eq_without_eq)]
#[derive(Clone, PartialEq, Eq, Hash, ::prost::Message)]
pub struct VerifyChallengeResponse {
    /// Session token for subsequent API calls
    #[prost(string, tag = "1")]
    pub session_token: ::prost::alloc::string::String,
    /// Token expiration timestamp (UTC Unix seconds)
    #[prost(int64, tag = "2")]
    pub expiration_timestamp: i64,
}
/// Generated client implementations.
pub mod spark_authn_service_client {
    #![allow(
        unused_variables,
        dead_code,
        missing_docs,
        clippy::wildcard_imports,
        clippy::let_unit_value,
    )]
    use tonic::codegen::*;
    use tonic::codegen::http::Uri;
    #[derive(Debug, Clone)]
    pub struct SparkAuthnServiceClient<T> {
        inner: tonic::client::Grpc<T>,
    }
    impl<T> SparkAuthnServiceClient<T>
    where
        T: tonic::client::GrpcService<tonic::body::Body>,
        T::Error: Into<StdError>,
        T::ResponseBody: Body<Data = Bytes> + std::marker::Send + 'static,
        <T::ResponseBody as Body>::Error: Into<StdError> + std::marker::Send,
    {
        pub fn new(inner: T) -> Self {
            let inner = tonic::client::Grpc::new(inner);
            Self { inner }
        }
        pub fn with_origin(inner: T, origin: Uri) -> Self {
            let inner = tonic::client::Grpc::with_origin(inner, origin);
            Self { inner }
        }
        pub fn with_interceptor<F>(
            inner: T,
            interceptor: F,
        ) -> SparkAuthnServiceClient<InterceptedService<T, F>>
        where
            F: tonic::service::Interceptor,
            T::ResponseBody: Default,
            T: tonic::codegen::Service<
                http::Request<tonic::body::Body>,
                Response = http::Response<
                    <T as tonic::client::GrpcService<tonic::body::Body>>::ResponseBody,
                >,
            >,
            <T as tonic::codegen::Service<
                http::Request<tonic::body::Body>,
            >>::Error: Into<StdError> + std::marker::Send + std::marker::Sync,
        {
            SparkAuthnServiceClient::new(InterceptedService::new(inner, interceptor))
        }
        /// Compress requests with the given encoding.
        ///
        /// This requires the server to support it otherwise it might respond with an
        /// error.
        #[must_use]
        pub fn send_compressed(mut self, encoding: CompressionEncoding) -> Self {
            self.inner = self.inner.send_compressed(encoding);
            self
        }
        /// Enable decompressing responses.
        #[must_use]
        pub fn accept_compressed(mut self, encoding: CompressionEncoding) -> Self {
            self.inner = self.inner.accept_compressed(encoding);
            self
        }
        /// Limits the maximum size of a decoded message.
        ///
        /// Default: `4MB`
        #[must_use]
        pub fn max_decoding_message_size(mut self, limit: usize) -> Self {
            self.inner = self.inner.max_decoding_message_size(limit);
            self
        }
        /// Limits the maximum size of an encoded message.
        ///
        /// Default: `usize::MAX`
        #[must_use]
        pub fn max_encoding_message_size(mut self, limit: usize) -> Self {
            self.inner = self.inner.max_encoding_message_size(limit);
            self
        }
        /// Request a new authentication challenge for a public key
        pub async fn get_challenge(
            &mut self,
            request: impl tonic::IntoRequest<super::GetChallengeRequest>,
        ) -> std::result::Result<
            tonic::Response<super::GetChallengeResponse>,
            tonic::Status,
        > {
            self.inner
                .ready()
                .await
                .map_err(|e| {
                    tonic::Status::unknown(
                        format!("Service was not ready: {}", e.into()),
                    )
                })?;
            let codec = tonic_prost::ProstCodec::default();
            let path = http::uri::PathAndQuery::from_static(
                "/spark_authn.SparkAuthnService/get_challenge",
            );
            let mut req = request.into_request();
            req.extensions_mut()
                .insert(
                    GrpcMethod::new("spark_authn.SparkAuthnService", "get_challenge"),
                );
            self.inner.unary(req, path, codec).await
        }
        /// Verify a signed challenge and return a session token
        pub async fn verify_challenge(
            &mut self,
            request: impl tonic::IntoRequest<super::VerifyChallengeRequest>,
        ) -> std::result::Result<
            tonic::Response<super::VerifyChallengeResponse>,
            tonic::Status,
        > {
            self.inner
                .ready()
                .await
                .map_err(|e| {
                    tonic::Status::unknown(
                        format!("Service was not ready: {}", e.into()),
                    )
                })?;
            let codec = tonic_prost::ProstCodec::default();
            let path = http::uri::PathAndQuery::from_static(
                "/spark_authn.SparkAuthnService/verify_challenge",
            );
            let mut req = request.into_request();
            req.extensions_mut()
                .insert(
                    GrpcMethod::new("spark_authn.SparkAuthnService", "verify_challenge"),
                );
            self.inner.unary(req, path, codec).await
        }
    }
}
